...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	if (UiTimer != none)
	{
		if (UiTimer.ShouldShow)
		{
			...
			//grimy changes, converted from generic counter to counter 2
			if ( class'GrimyHighlanderConfig'.default.SHIFT_MISSION_TIMERS ) {
				m_kTurnCounter2.SetUIState(UiTimer.UiState);
				m_kTurnCounter2.SetLabel(UiTimer.DisplayMsgTitle);
				m_kTurnCounter2.SetSubLabel(UiTimer.DisplayMsgSubtitle);
				m_kTurnCounter2.SetCounter(string(UiTimer.TimerValue));
			}
			else {
