...
//---------------------------------------------------------------------------------------
class X2AbilityToHitCalc extends Object config(GrimyHighlander)
	abstract;

// Start of Grimy Additions
var config bool ENABLE_OVERWATCH_TWEAK;
var config bool ADDITIVE_DODGE, SCALE_DODGE_TO_MISS;
var config float SCALE_DODGE_TO_MISS_MULT;
...
protected function FinalizeHitChance()
{
	...
	{
		// Start of Grimy Additions
		if ( default.SCALE_DODGE_TO_MISS )
		{
			GrazeScale = float(m_ShotBreakdown.ResultTable[eHit_Graze]) / 100.0f;
			GrazeScale *= float(100 - m_ShotBreakdown.FinalHitChance);
			GrazeScale *= default.SCALE_DODGE_TO_MISS_MULT;
			FinalGraze = Round(GrazeScale);
			FinalGraze = min(m_ShotBreakdown.ResultTable[eHit_Success],FinalGraze);
			m_ShotBreakdown.ResultTable[eHit_Success] -= FinalGraze;
			m_ShotBreakdown.ResultTable[eHit_Graze] = FinalGraze;
		}
		else if ( default.ADDITIVE_DODGE )
		{
