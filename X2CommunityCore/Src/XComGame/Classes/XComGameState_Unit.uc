...
simulated function ApplyCombatSimStats(XComGameState_Item CombatSim, optional bool bLookupBonus = true, optional bool bForceBonus = false)
{
	...
	for(i = 0; i < CombatSim.StatBoosts.Length; i++)
	{
		...
		{
			// prevent willpower from being restored after a mission if morale mode is activated
			if ( !(CombatSim.StatBoosts[i].StatType == eStat_Will && class'GrimyHighlanderConfig'.default.MORALE_MODE) ) {
...
function RankUpSoldier(XComGameState NewGameState, optional name SoldierClass, optional bool bRecoveredFromBadClassData, optional bool bRoulette = false)
{
	...
	if (m_SoldierRank == 0)
	{
		...
		if ( ( class'Grimy_Utility_TrainingRoulette'.default.RouletteAllSoldiersChance > `SYNC_RAND(100) || bRoulette ) && SoldierClass != 'Spark' && SoldierClass != 'PsiOperative' && SoldierClass != 'CentralOfficer' ) {
	...
	if (RankIndex >= 0 && RankIndex < Template.GetMaxConfiguredRank())
	{
		...
		for (i = 0; i < StatProgression.Length; ++i)
		{
			...
			{				
				...
				{
					// prevent willpower from being restored after a mission if morale mode is activated.
					if ( !(StatProgression[i].StatType == eStat_Will && class'GrimyHighlanderConfig'.default.MORALE_MODE) ) {
