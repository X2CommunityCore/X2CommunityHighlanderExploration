...
function EventListenerReturn OnSpawnReinforcementsComplete(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
	...
	// Grimy Changes to this if/else. I need the group to do stuff even if there are no visible units.
	...
	{
		AIGroupState = ReinforcementUnit.GetGroupMembership();
		InstigatingUnit = GetNearestEnemy(AIGroupState.GetGroupMidpoint());
		AIGroupState.InitiateReflexMoveActivate(InstigatingUnit, eAC_SeesSpottedUnit);
...
function XComGameState_Unit GetNearestEnemy(vector TargetLocation) {
	local float fDistSq, fClosest;
	local XComGameStateHistory History;
	local XComGameState_Unit UnitState, ClosestUnit;
	...
	foreach History.IterateByClassType(class'XComGameState_Unit', UnitState)
	{
		if( UnitState.GetTeam() == eTeam_XCom && !UnitState.bRemovedFromPlay && UnitState.IsAlive() && !UnitState.IsBleedingOut() )
		{
			...
			if( ClosestUnit.ObjectID <= 0 || fDistSq < fClosest )
			{
